...
{{<figure `Transistors` `/images/SE1.transistors.png` right 40 >}}
...
The P-type substrate prevents current to flow from the source (left) to the drain (right). When voltage is applied to the gate, the positive charge in the dielectric layer induces a field in the P-type substrate which allows electrons to flow to the drain.

{{<figure `Transistor Symbol` `/images/SE1.transistor.png` right 40 >}}
...
{{<figure `MOSFET` `/images/SE1.MOSFET.png` right 40 >}}
...
In this blog we are interested in the (logical) system engineering rather than electronic engineering, so we use simplified design.

# Digital Computers

{{<figure `AND-Gate` `/images/SE1.AND-gate.png` right 40 >}}
...
* as we will learn, all Boolean operators can be computed using others. For instance, the NAND can compute *all other operators*
