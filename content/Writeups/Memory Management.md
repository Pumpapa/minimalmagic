...
When software isn't overly complex, an object's end of life follows from the logic. At that time, a handler can be called to reclaim the memory. This is called **explicit** memory management.

But in many realistic situations it is impossible to determine the moment after which an object is no longer needed (the question whether a program will use some object in the future is undecidable in general). 
...
Actually 'automatic' is a misnomer: software libraries are used to govern access and administer state, hiding management from the programmer. This is impossible in general, but effective in specific cases such as abstract machines. 
